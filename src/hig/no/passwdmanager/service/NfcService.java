...
import hig.no.passwdmanager.util.CryptoUtils;
 ...
 * <li><code>EXTERNAL AUTHENTICATE</code> (also using secure messaging for EAC)</li>
 ...
 * 
 */
public class NfcService extends CardService {

	private static final long serialVersionUID = 8071684120261079443L;
	
	private static final String TAG = "NFC APDU";
	...
	public NfcService(IsoDep picc) throws CardServiceException{
		this.picc = picc;
				try {				
	...
	public ResponseAPDU transmit(CommandAPDU apdu) throws CardServiceException {
		try {
			byte [] rapdu = picc.transceive(apdu.getBytes());
			Log.v(TAG+"c", Hex.bytesToHexString(apdu.getBytes()));
			Log.v(TAG+"r", Hex.bytesToHexString(rapdu));
	...
	private void sendSelectApplet() throws CardServiceException {
        ...
        if (sw != 0x00009000) {
            throw new CardServiceException("Could not select card");
        }
    }
     ...
     * Possibly use secure messaging (EAC).
     * 
